...
public class ImportAlunoImpl {
	...
	public ImportAlunoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportAlunoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_ALUNO, //
	...
	*/
	public ImportAlunoOutput importAluno(ImportAlunoInput input, long timeout) {
		...
		ImportAlunoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportAlunoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportAlunoOutput> importAlunoRequest(ImportAlunoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportAlunoOutput.class);
	}
