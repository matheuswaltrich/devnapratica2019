...
public class ExportAlunoImpl {
	...
	public ExportAlunoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportAlunoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_ALUNO, //
	...
	*/
	public ExportAlunoOutput exportAluno(ExportAlunoInput input, long timeout) {
		...
		ExportAlunoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportAlunoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportAlunoOutput> exportAlunoRequest(ExportAlunoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportAlunoOutput.class);
	}
