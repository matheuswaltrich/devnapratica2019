...
public class ListAlunoImpl {
	...
	public ListAlunoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Aluno.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_ALUNO, //
	...
	*/
	public Aluno.PagedResults listAluno(Aluno.PageRequest input, long timeout) {
		...
		Aluno.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Aluno.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Aluno.PagedResults> listAlunoRequest(Aluno.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Aluno.PagedResults.class);
	}
