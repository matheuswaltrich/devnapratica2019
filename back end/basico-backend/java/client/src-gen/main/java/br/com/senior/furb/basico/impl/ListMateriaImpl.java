...
public class ListMateriaImpl {
	...
	public ListMateriaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Materia.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_MATERIA, //
	...
	*/
	public Materia.PagedResults listMateria(Materia.PageRequest input, long timeout) {
		...
		Materia.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Materia.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Materia.PagedResults> listMateriaRequest(Materia.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Materia.PagedResults.class);
	}
