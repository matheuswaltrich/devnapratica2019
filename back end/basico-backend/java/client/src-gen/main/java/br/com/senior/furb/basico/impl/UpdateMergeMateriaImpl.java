 ...
 * 
 */
public class UpdateMergeMateriaImpl {
	...
	public UpdateMergeMateriaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Materia input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.UPDATE_MERGE_MATERIA, //
	...
	*/
	public Materia updateMergeMateria(Materia input, long timeout) {
		...
		Materia output = DtoJsonConverter.toDTO(resultMessage.getContent(), Materia.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Materia> updateMergeMateriaRequest(Materia input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Materia.class);
	}
