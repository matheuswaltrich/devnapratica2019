...
public class ImportTurmaImpl {
	...
	public ImportTurmaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportTurmaInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_TURMA, //
	...
	*/
	public ImportTurmaOutput importTurma(ImportTurmaInput input, long timeout) {
		...
		ImportTurmaOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportTurmaOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportTurmaOutput> importTurmaRequest(ImportTurmaInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportTurmaOutput.class);
	}
