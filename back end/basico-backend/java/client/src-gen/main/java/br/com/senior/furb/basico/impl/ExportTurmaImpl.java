...
public class ExportTurmaImpl {
	...
	public ExportTurmaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportTurmaInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_TURMA, //
	...
	*/
	public ExportTurmaOutput exportTurma(ExportTurmaInput input, long timeout) {
		...
		ExportTurmaOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportTurmaOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportTurmaOutput> exportTurmaRequest(ExportTurmaInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportTurmaOutput.class);
	}
