 ...
 * 
 */
public class CreateMergeAlunoImpl {
	...
	public CreateMergeAlunoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Aluno input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.CREATE_MERGE_ALUNO, //
	...
	*/
	public Aluno createMergeAluno(Aluno input, long timeout) {
		...
		Aluno output = DtoJsonConverter.toDTO(resultMessage.getContent(), Aluno.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Aluno> createMergeAlunoRequest(Aluno input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Aluno.class);
	}
