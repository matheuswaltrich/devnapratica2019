...
public class ListTurmaImpl {
	...
	public ListTurmaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Turma.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_TURMA, //
	...
	*/
	public Turma.PagedResults listTurma(Turma.PageRequest input, long timeout) {
		...
		Turma.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Turma.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Turma.PagedResults> listTurmaRequest(Turma.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Turma.PagedResults.class);
	}
