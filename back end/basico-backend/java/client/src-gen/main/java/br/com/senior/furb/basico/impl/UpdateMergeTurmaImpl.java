 ...
 * 
 */
public class UpdateMergeTurmaImpl {
	...
	public UpdateMergeTurmaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Turma input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.UPDATE_MERGE_TURMA, //
	...
	*/
	public Turma updateMergeTurma(Turma input, long timeout) {
		...
		Turma output = DtoJsonConverter.toDTO(resultMessage.getContent(), Turma.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Turma> updateMergeTurmaRequest(Turma input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Turma.class);
	}
